...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!user) {
      ...
      return res.status(200).json({ ok: true, status: "confirmed", reference: pay.reference, credited: acreditado, saldo: acreditado, tx });
    } else {
      ...
      return res.status(200).json({ ok: true, status: "confirmed", reference: pay.reference, credited: acreditado, saldo: nuevoSaldo, tx });
    }
  } catch (e: any) {
